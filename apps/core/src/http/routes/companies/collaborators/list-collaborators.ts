...
/*
 * Filtro da listagem do Time. Acompanha os papéis do domain — filtrar por um
 * papel que existe e receber 400 é o mesmo defeito que a escrita teve quando
 * `admin` e `recruiter` entraram sem passar por aqui. (`shared` nunca esteve
 * na lista; entra agora, porque não filtrar por convidado não tem razão.)
 */
const accessLevelEnum = z.enum(['all', 'owner', 'admin', 'recruiter', 'editor', 'shared'])

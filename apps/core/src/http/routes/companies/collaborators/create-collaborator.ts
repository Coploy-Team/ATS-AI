			...
			'/companies/collaborators',
			{
				schema: {
					...
					body: z.object({
						...
						name: z.string(),
						/*
						 * Convite. `owner` fica de fora de propósito — quem responde
						 * pela conta não nasce de um convite — e `editor` também, por
						 * ser legado: ele existe em quem já tem, não se cria mais.
						 */
						accessLevel: z.enum(['admin', 'recruiter', 'shared']),
						status: z.boolean(),
						password: z.string(),
					}),
					response: {

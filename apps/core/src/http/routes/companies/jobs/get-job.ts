...
import { isJobInScope } from '@/lib/access-scope'
  ...
  app
    ...
    .get<{
          ...
          response: {
            200: z
              .object({
                ...
                /*
                 * Anulável de propósito: a vaga que nunca teve a chave tocada
                 * não tem valor gravado, e a tela precisa distinguir "não
                 * decidiu" (mostra ligado, que é o padrão) de um `false`
                 * explícito.
                 */
                sendCandidateFeedback: z.boolean().nullish(),
      ...
      async (request) => {
        ...
        /*
         * Fora do alcance é a MESMA resposta de inexistente: um recrutador que
         * recebe "existe, mas não é sua" já sabe que a vaga sigilosa existe.
         */
        if (project && !isJobInScope(request, project)) {
        ...
        return {
          ...
          /*
           * `?? null` e não `=== true`: aqui "nunca decidiu" precisa chegar
           * distinguível do `false` explícito. A tela mostra ligado no primeiro
           * caso (é o padrão) e desligado no segundo — comparar com `true`
           * transformaria vaga antiga em "retorno desligado" sem ninguém pedir.
           */
          sendCandidateFeedback: project?.sendCandidateFeedback ?? null,

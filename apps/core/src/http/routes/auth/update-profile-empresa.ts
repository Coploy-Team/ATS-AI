	...
	app
		...
		.patch(
			'/profile',
			{
				schema: {
					...
					body: z.object({
						 ...
						 *
						 * Mora aqui, e não numa tela de administração, porque é escolha
						 * de cada pessoa — ninguém precisa pedir para o gestor parar de
						 * receber e-mail. Substitui a lista negra do Postmark, que
						 * silenciava o endereço inteiro.
						 */
						notifyOnInterviewFinish: z.boolean().optional(),
					}),
					response: {
						200: z.object({
							user: z.object({
								...
								/** Ausente na base = recebe; a tela mostra ligado. */
			...
			async (request, reply) => {
				...
				/** Preenchido no espelho abaixo — a resposta devolve o estado gravado. */
				let colaboradorAtual: Record<string, unknown> | null = null

				const { name, photoUrl, phoneNumber, notifyOnInterviewFinish } = request.body
				 ...
				 */
				/*
				 * A preferência de aviso vive no COLABORADOR, não em `users`: ela é
				 * por empresa. A mesma pessoa pode acompanhar as vagas de uma e não
				 * querer saber das outras.
				 */
				if (name !== undefined || notifyOnInterviewFinish !== undefined) {
					try {
						...
						if (mine?.id) {
							await app.infra.collaboratorRepository.updateCollaborator(
								...
								{
									...(name !== undefined && { name }),
									...(notifyOnInterviewFinish !== undefined && { notifyOnInterviewFinish }),
								} as never,
							)
						}
						colaboradorAtual = mine ?? null
					} catch (error) {
						console.warn(
				...
				return reply.status(200).send({
					user: {
						...
						/*
						 * O valor que a tela deve mostrar. Prioridade: o que acabou de
						 * ser enviado, depois o que estava gravado, e ausente = LIGADO —
						 * campo novo não pode aparecer desligado para quem nunca mexeu.
						 */
						notifyOnInterviewFinish:
							notifyOnInterviewFinish ??
							(colaboradorAtual?.notifyOnInterviewFinish as boolean | undefined) ??
							true,
					},
				})
			},

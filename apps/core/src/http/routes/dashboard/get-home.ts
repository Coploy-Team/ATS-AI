	...
	app
		...
		.get(
			...
			async (request) => {
				...
				// Generate cache key
				/*
				 * O alcance entra na CHAVE do cache. Sem isso a primeira pessoa a
				 * abrir o painel definiria o que as outras veriam — o recrutador
				 * serviria os próprios números ao gestor, e o gestor serviria a
				 * empresa inteira ao recrutador.
				 */
				const alcance = await dashboardScope(app.infra, request, company.id)
				const cacheKey = `${getCacheKey(company.id, month, year)}:${alcance.userId ?? 'todos'}`

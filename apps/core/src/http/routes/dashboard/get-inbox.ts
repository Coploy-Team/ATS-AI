	...
	app
		...
		.post(
			...
			async (request) => {
				...
				/*
				 * A CHAVE DO CACHE precisa carregar o alcance.
				 *
				 * O inbox é guardado por empresa e traz nome de candidato e vaga.
				 * Com o recorte por recrutador e a chave só com o `companyId`, a
				 * primeira pessoa a abrir a tela definiria o que todas as outras
				 * veriam — um recrutador serviria a própria caixa para o gestor, e
				 * o gestor serviria a empresa inteira para o recrutador.
				 */
				const chaveDeCache = alcance.userId ? `${companyId}:${alcance.userId}` : companyId
				...
				const cached = inboxCache.get(chaveDeCache)

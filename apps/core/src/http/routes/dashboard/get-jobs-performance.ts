	...
	app
		...
		.post(
			...
			async (request) => {
				...
				// o alcance entra na chave: sem isso o painel de um recrutador
				// serviria de cache para o gestor (e vice-versa)
				const cacheKey = `jobs-performance:${companyId}:${alcance.userId ?? 'todos'}:${monthKey}`

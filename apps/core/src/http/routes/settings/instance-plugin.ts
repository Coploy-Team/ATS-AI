 ...
 * . Mesmo gate da seção de e-mail: distribuição open + dono.
 *
...
const licenseStatusSchema = z.object({
	...
	/** Onde baixar o compose do plugin com a licença (instalação guiada, fase 2). */
	licenseServerUrl: z.string(),
	/**
	 * Estado vivo dos serviços do Motor (decisão 3 do ADR-009) — null quando o
	 * Motor não está instalado. `interviewBaseUrl` vai pra tela pingar a sala
	 * DO BROWSER (ela é endereço público do host; o core não a alcança).
	 */
	services: z
		.object({
			engine: z.boolean(),
			orchestrator: z.boolean(),
			interviewBaseUrl: z.string().nullable(),
		})
		.nullable(),
})
...
/** Ping curto num serviço interno do plugin — vivo = respondeu qualquer coisa. */
async function pingService(url: string | undefined): Promise<boolean> {
	if (!url) return false
	try {
		const response = await fetch(url, { signal: AbortSignal.timeout(2_000) })
		return response.status < 500 || response.status === 500
	} catch {
		return false
	}
}

/** Cache curto: a tela re-renderiza à vontade sem virar health-check storm. */
let servicesCache: { at: number; value: { engine: boolean; orchestrator: boolean } } | null = null

async function motorServicesStatus(): Promise<{
	engine: boolean
	orchestrator: boolean
	interviewBaseUrl: string | null
} | null> {
	if ((env as { MOTOR_ENABLED?: boolean }).MOTOR_ENABLED !== true) return null
	if (!servicesCache || Date.now() - servicesCache.at > 15_000) {
		const [engine, orchestrator] = await Promise.all([
			pingService((env as { ENGINE_URL?: string }).ENGINE_URL),
			pingService((env as { INTEGRATION_URL?: string }).INTEGRATION_URL),
		])
		servicesCache = { at: Date.now(), value: { engine, orchestrator } }
	}
	return {
		...servicesCache.value,
		interviewBaseUrl: (env as { INTERVIEW_BASE_URL?: string }).INTERVIEW_BASE_URL ?? null,
	}
}
	...
	typed.get(
		...
		async (request) => {
			...
			return {
				...
				licenseServerUrl: env.MOTOR_LICENSE_SERVER_URL,
				services: await motorServicesStatus(),
			}
		},
	)

import type { Capability, JobScope, TenantRole } from '@coploy/domain'
import { can, jobScopeOf, normalizeTenantRole } from '@coploy/domain'
...
export function registerRbac(app: FastifyInstance, infra: InfraProvider) {
	/*
	 * O papel resolvido fica no request para quem PRECISA dele além do guard.
	 *
	 * Não é permissão, é ALCANCE: o recrutador pode ler vaga — só não todas.
	 * Filtrar é trabalho do serviço, e o serviço precisa saber de quem é a
	 * sessão. Sem isto, cada rota interessada listaria os colaboradores de novo
	 * para descobrir o mesmo papel que este hook acabou de resolver.
	 *
	 * Vale mesmo com o enforcement DESLIGADO: escopo não nega ação, delimita
	 * conjunto — e um recrutador vendo a vaga alheia é o defeito que o cliente
	 * relatou, não um alarme de auditoria.
	 */
	app.decorateRequest('tenantRole', null)
	app.decorateRequest('tenantUserId', null)
	/*
	...
	app.addHook('onRoute', (route: RouteLike) => {
		...
		const guard = async (request: FastifyRequest, reply: FastifyReply) => {
			...
			if (!capability) {
				/*
				 * Mesmo sem capability mapeada o papel é resolvido: o escopo não
				 * depende da tabela de política, e deixar a rota sem `tenantRole`
				 * faria o serviço cair no default "vê tudo" — abrindo justamente
				 * o que a rota não mapeada tem de mais arriscado.
				 */
				;(request as { tenantRole?: TenantRole | null }).tenantRole = await resolveRole(
					infra,
				...
				;(request as { tenantUserId?: string | null }).tenantUserId = userId
			...
			;(request as { tenantRole?: TenantRole | null }).tenantRole = role
			;(request as { tenantUserId?: string | null }).tenantUserId = userId
...
/**
 * Alcance da sessão sobre as vagas.
 *
 * `all` quando o papel não foi resolvido — mesma decisão de
 * `normalizeTenantRole`: ausência de papel é a base legada inteira, e tratá-la
 * como restrita trancaria o cliente fora do próprio dado no dia do deploy.
 * Quem quer restringir atribui `recruiter` explicitamente.
 */
export function jobScopeOfRequest(request: FastifyRequest): {
	scope: JobScope
	userId: string | null
} {
	const role = (request as { tenantRole?: TenantRole | null }).tenantRole ?? null
	const userId = (request as { tenantUserId?: string | null }).tenantUserId ?? null
	if (!role || !userId) return { scope: 'all', userId }
	return { scope: jobScopeOf(role), userId }
}

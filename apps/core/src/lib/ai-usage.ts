...
export function recordCoreAiUsage(input: {
	...
	/*
	 * Sem medidor, não se mede.
	 *
	 * Medir custo de IA por empresa é necessidade de quem opera um serviço com
	 * muitas empresas. Numa instalação que roda para si mesma o medidor não
	 * existe, e antes disto o caminho de geração precisava saber disso — cada
	 * rota de IA carregava a telemetria como se fosse parte do produto.
	 */
	const medidor = infra.aiUsageRepository
	if (!medidor) return
	const now = new Date()

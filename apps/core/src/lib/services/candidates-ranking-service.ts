...
export function createCandidatesRankingService(infra: InfraProvider) {
	...
	async function fetchInterviewsWithTextSearchProgressive(
		...
		while (
			...
			const filteredBatch = batchInterviews.filter((interview) => {
			...
			// Atualizar cursor
			if (batchInterviews.length > 0) {
				const lastInterview = batchInterviews[batchInterviews.length - 1]
			...
			hasMoreData = batchInterviews.length >= BATCH_SIZE
	...
	async function fetchInterviewBatch(
		...
		return infra.candidateRepository.listCompanyInterviews(
			companyId,
			batchOptions,

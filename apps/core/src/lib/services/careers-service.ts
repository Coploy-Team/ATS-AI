...
export interface CareersJobSummary {
	...
	interviewUrl: string
	/**
	 * A vaga está PRONTA pra entrevistar (Motor presente + perguntas
	 * cadastradas) — decisão 2 do martelo de 2026-08-23. A tela só oferece
	 * "fazer entrevista" quando true; o orchestrator recusa de todo jeito.
	 */
...
function toSummary(companyId: string, job: PostJob, shortApplicationForm: boolean): CareersJobSummary {
	...
	const questionCount = (job.jobQuestions?.length ?? 0) + (job.additionalQuestions?.length ?? 0)
	const interviewReady = getInstallationFeatures().motor && questionCount > 0
	return {
		jobId: job.id,
		companyId,
		title: job.jobName ?? '',

...
export function createInterviewsService(
	...
	return {
		...
		async listInterviews(params: {
			...
			groupBy?: 'candidate'
			/**
			 * Vagas que a sessão alcança, ou `null` para todas.
			 *
			 * É a mesma parede da listagem de vagas, aplicada à lista de
			 * entrevistas — "eles têm visão de todas as entrevistas" foi
			 * literalmente a frase do cliente. Esconder a vaga e deixar a
			 * entrevista dela aparecer aqui devolveria pela janela o que a porta
			 * fechou: o nome do candidato, a nota e a vaga estão nesta linha.
			 */
			jobIdsInScope?: Set<string> | null
		}) {
			const { companyId, page, limit, find, status, minScore, from, to, groupBy } = params
			const escopoDeVagas = params.jobIdsInScope ?? null
			/** Filtra pelo alcance quando ele existe; caminho normal não paga nada. */
			const dentroDoEscopo = <T extends { post_job_id?: string | null }>(itens: T[]): T[] =>
				escopoDeVagas === null
					? itens
					: itens.filter((item) => interviewInScope(item, escopoDeVagas))
			...
			if (shouldUseOptimizedQuery) {
				...
				/*
				 * O recorte por alcance vem ANTES do corte de página: filtrar
				 * depois devolveria páginas curtas e um "tem mais" mentiroso.
				 */
				const permitidas = dentroDoEscopo(interviewsWithExtra)
				const hasMore = permitidas.length > limit
				const actual = hasMore ? permitidas.slice(0, limit) : permitidas
			...
			const interviews = dentroDoEscopo(
		...
		async getCandidateDetails(params: {
			...
			/**
			 * Vagas que a sessão alcança; `null` = todas.
			 *
			 * O dossiê do candidato lista TODAS as entrevistas dele na empresa —
			 * é a tela onde o nome das vagas dos colegas aparece uma a uma, com
			 * nota. Recortar a lista de candidatos e deixar esta aberta devolve
			 * pelo detalhe o que a lista escondeu.
			 */
			jobIdsInScope?: Set<string> | null
		}) {
			const { userId, companyId, company } = params
			const alcance = params.jobIdsInScope ?? null
			...
			/*
			 * Recorte da sessão — depois de juntar as duas buscas (por usuário e
			 * por e-mail) e ANTES de ordenar: assim vale para tudo que a tela
			 * mostra, inclusive as médias calculadas abaixo.
			 */
			const alcancadas = interviews.filter((interview) => interviewInScope(interview, alcance))

			const sortedInterviews = [...alcancadas].sort((a, b) => {

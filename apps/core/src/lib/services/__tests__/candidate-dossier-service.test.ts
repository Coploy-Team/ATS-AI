...
describe('createCandidateDossierService', () => {
	...
	describe('bloqueio SaaS (V2-704)', () => {
		it('empresa SaaS sem crédito recebe o dossiê bloqueado', async () => {
			infra.companyRepository.getCompany.mockResolvedValue({
				id: COMPANY_ID,
				subscriptionPlan: 'pro',
			} as never)

			const dossier = await createCandidateDossierService(infra).getDossier({
				companyId: COMPANY_ID,
				jobId: JOB_ID,
				candidateId: CANDIDATE_ID,
			})

			expect(dossier.locked).toBe(true)
			// o que se compra some…
			expect(dossier.interview?.score).toBeNull()
			expect(dossier.interview?.questions).toEqual([])
			expect(dossier.candidate.email).toBeNull()
			// …e o que permite decidir se vale desbloquear continua
			expect(dossier.candidate.name).toBe('Mariana Vidal')
			expect(dossier.application.stage).toBe('selected')
			expect(dossier.interview?.questionCount).toBeGreaterThan(0)
		})

		it('enterprise nunca é bloqueada', async () => {
			const dossier = await createCandidateDossierService(infra).getDossier({
				companyId: COMPANY_ID,
				jobId: JOB_ID,
				candidateId: CANDIDATE_ID,
			...
			expect(dossier.locked).toBe(false)
			expect(dossier.interview?.score).toBe(8.6)
		})

		it('falha ao ler a empresa mantém o bloqueio — fail closed', async () => {
			infra.companyRepository.getCompany.mockRejectedValue(new Error('firestore down'))

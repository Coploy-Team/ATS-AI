...
describe('createCandidatesRankingService', () => {
	...
	/**
	 * A vaga da entrevista vem em DOIS formatos.
	 *
	 * O espelho grava `post_job_id` em alguns registros e `job_ref` em outros —
	 * o código já convivia com isso no admin. Meu recorte olhava só o primeiro,
	 * e toda entrevista gravada com a referência era descartada: o candidato
	 * sumia da tela de quem criou a vaga. Foi o defeito relatado.
	 */
	describe('alcance do recrutador', () => {
		const base = {
			finished: true,
			score: '9.0',
			date: new Date('2024-02-01T10:00:00.000Z').toISOString(),
		...
		async function buscarCom(entrevistas: unknown[]) {
			infra.candidateRepository.listCompanyInterviews.mockResolvedValue(entrevistas as never)
			return service.fetchCandidatesInBatches(
				COMPANY_ID,
				1,
				12,
				{
					...
					dataRange: 'all',
					interviewCount: 'all',
					jobIdsInScope: new Set(['minha-vaga']),
				} as never,
				true,
			)
		}

		it('acha a entrevista da própria vaga quando a vaga vem por job_ref', async () => {
			const resultado = await buscarCom([
				{ ...base, id: 'minha-1', name: 'Candidata', email: 'minha@x.com', job_ref: { id: 'minha-vaga' } },
			])
			expect(resultado.candidates.map((c) => c.email)).toContain('minha@x.com')
		})

		it('acha também quando vem por post_job_id', async () => {
			const resultado = await buscarCom([
				{ ...base, id: 'minha-2', name: 'Candidata', email: 'minha2@x.com', post_job_id: 'minha-vaga' },
			])
			expect(resultado.candidates.map((c) => c.email)).toContain('minha2@x.com')
		})

		it('e continua sem devolver o candidato da vaga alheia', async () => {
			const resultado = await buscarCom([
				{ ...base, id: 'alheia-1', name: 'Do colega', email: 'colega@x.com', job_ref: { id: 'vaga-do-colega' } },
				{ ...base, id: 'alheia-2', name: 'Do colega 2', email: 'colega2@x.com', post_job_id: 'vaga-do-colega' },
			])
			expect(resultado.candidates).toHaveLength(0)
		})
	})
})
...
describe('createInterviewInviteService', () => {
	...
	beforeEach(() => {
		...
		infra.jobRepository.getJob.mockResolvedValue({
			...
			// decisão 2 (martelo 2026-08-23): vaga sem pergunta não convida —
			// o mock precisa ser entrevistável pros cenários de convite
			jobQuestions: [{ id: 'q1', question: 'Conte um projeto' }],
		} as never)
	...
	it('recusa convite pra vaga sem perguntas — a sala não pode receber ninguém', async () => {
		infra.jobRepository.getJob.mockResolvedValue({
			id: JOB_ID,
			jobName: 'Engenheira de Software',
			jobQuestions: [],
		...
		).rejects.toThrow('não tem perguntas')
	})

	it('não convida quem já terminou a entrevista — nem e-mail, nem volta de etapa', async () => {
		infra.candidateRepository.getJobInterview.mockResolvedValue(
			interviewDoc({ finished: true, candidateStatus: 'Applied' }) as never,
		)
		...
		// o estrago não era só o e-mail: mover devolvia a ficha pra "Entrevista IA"
		// e reiniciava o relógio da etapa de quem já tinha resultado

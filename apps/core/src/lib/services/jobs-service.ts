...
function buildJobFilters(filters: JobFilters): QueryFilter[] {
  ...
  // Só expor WhatsApp no dashboard: docs antigos não têm interviewMode,
  // então where('interviewMode', '==', 'video') deixaria vagas legadas de fora.
  if (filters.interviewMode) {
    queryFilters.push({ field: 'interviewMode', operator: '==', value: filters.interviewMode })
  }

  /*
   * Alcance da sessão vem ANTES do filtro do cliente e vence sempre.
   *
   * O recrutador enxerga só o que criou. Se o `creatorId` do cliente mandasse
   * aqui, bastaria trocar o parâmetro na URL para ver a vaga de outra pessoa —
   * a parede tem que ser do servidor, não da tela.
   */
  if (filters.scope === 'own' && filters.scopedToUserId) {
    queryFilters.push({ field: 'creatorId', operator: '==', value: filters.scopedToUserId })
  } else if (filters.creatorId && filters.creatorId !== 'all') {

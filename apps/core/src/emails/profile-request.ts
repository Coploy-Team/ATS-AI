...
export function renderProfileRequestEmail(params: ProfileRequestEmailParams): RenderedEmail {
	...
	const bodyHtml = `
		...
		<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="Margin:20px 0 0 0;border-collapse:collapse;">
			<tr>
				<td style="background-color:${accentOf(params.branding)};">
					<a href="${escapeHtml(params.profileUrl)}" style="display:inline-block;padding:13px 26px;font-family:${emailTheme.fontFamily};font-size:15px;font-weight:bold;color:${accentTextOf(params.branding)};text-decoration:none;">${escapeHtml(copy.cta)}</a>
				</td>
			</tr>
		</table>
		...
		<p style="Margin:14px 0 0 0;font-family:${emailTheme.fontFamily};font-size:14px;line-height:22px;color:${emailTheme.colors.mutedText};">${escapeHtml(copy.footer(params.branding?.companyName ?? 'Coploy'))}</p>`

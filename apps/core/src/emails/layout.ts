...
export function renderEmailLayout(params: EmailLayoutParams): string {
	const { previewText, title, bodyHtml, language = 'pt-BR' } = params
		...
		<tr>
			<td align="center" style="padding:28px 12px;">
				<table role="presentation" width="${emailTheme.maxWidth}" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:${emailTheme.maxWidth}px;background-color:${emailTheme.colors.surface};border-collapse:collapse;border:1px solid ${emailTheme.colors.border};">
					<tr>
						<td style="padding:26px 30px 18px 30px;border-bottom:4px solid ${emailTheme.colors.accent};background-color:${emailTheme.colors.surface};">
							<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
								<tr>
									<td>
										<img src="${escapeHtml(emailTheme.logoUrl)}" alt="${escapeHtml(emailTheme.brandName)}" width="136" style="display:block;border:0;outline:none;text-decoration:none;max-width:136px;height:auto;">
									</td>
								</tr>
							</table>
						</td>
					...
					<tr>
						<td style="padding:24px 30px;background-color:${emailTheme.colors.footer};font-family:${emailTheme.fontFamily};">
							<p style="Margin:0 0 8px 0;font-size:14px;line-height:20px;color:${emailTheme.colors.footerText};">Coploy</p>
							<p style="Margin:0;font-size:12px;line-height:18px;color:${emailTheme.colors.footerText};">Precisa falar com a gente? Escreva para <a href="mailto:${escapeHtml(emailTheme.supportEmail)}" style="color:${emailTheme.colors.footerText};text-decoration:underline;">${escapeHtml(emailTheme.supportEmail)}</a>.</p>
						</td>
					</tr>
				</table>
			</td>

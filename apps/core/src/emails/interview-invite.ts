 ...
 * (`docs/talent-os/gaps/research-01-gupy.md`, D03/D04).
 */
...
export function renderInterviewInviteEmail(params: InterviewInviteEmailParams): RenderedEmail {
	...
	const bodyHtml = `
							...
							<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="Margin:22px 0 0 0;border-collapse:collapse;">
								<tr>
									<td style="background-color:${emailTheme.colors.accent};">
										<a href="${escapeHtml(params.interviewUrl)}" style="display:inline-block;padding:13px 26px;font-family:Arial, Helvetica, sans-serif;font-size:15px;font-weight:bold;color:#ffffff;text-decoration:none;">${escapeHtml(copy.cta)}</a>
									</td>
								</tr>
							</table>
							...
							<p style="Margin:14px 0 0 0;font-family:Arial, Helvetica, sans-serif;font-size:14px;line-height:22px;color:#5d555a;">${escapeHtml(copy.footer)}</p>`
	...
	return {
		...
		textBody: [greeting, body, message, copy.howItWorks, params.interviewUrl, copy.footer]
			.filter(Boolean)
			.join('\n\n'),
	}
}

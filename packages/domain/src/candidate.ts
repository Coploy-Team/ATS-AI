...
export interface JobApplied {
	...
	/**
	 * Quando o pipeline de finalização foi RECLAMADO por alguém.
	 *
	 * `finished` só é gravado no fim, depois da análise — e a entrevista tem
	 * mais de um gatilho de finish (a última transcrição e, no WhatsApp, a
	 * chamada explícita do agente). Os dois liam `finished === false` e rodavam
	 * o pipeline inteiro em paralelo: candidato e recrutadores recebiam e-mail
	 * duplicado e a análise final era cobrada duas vezes. Confirmado em
	 * produção (MRV, 2026-08-28: dois `[Finish] Start` para a mesma entrevista
	 * com 12s de diferença).
	 *
	 * A reivindicação é atômica e tem validade: uma execução que morreu no meio
	 * libera o direito depois de alguns minutos, senão uma falha transitória
	 * deixaria a entrevista sem finalizar para sempre.
	 */
	finishStartedAt?: Date | null
	 ...
	 * apply e verificada por nós — consumo do padrão .
	 */

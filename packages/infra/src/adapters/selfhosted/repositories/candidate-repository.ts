...
function mapJobAppliedFromRow(raw: JobAppliedWithRelations): JobApplied {
	...
	/*
	 * NÃO usar stripInternal aqui: ele descarta `id` por padrão (pensado pra
	 * PK interna de linha), mas nas tabelas de resposta o id da linha É o id
	 * da PERGUNTA — sem ele, o answer do orchestrator não encontra a pergunta
	 * ("Question not found") e NENHUMA resposta é aceita no selfhosted.
	 * Terceiro membro da família do mapeamento campo a campo (2026-08-24).
	 */
	if (raw.answers?.length) {
		interview.info = raw.answers.map((a) => {
			const row = postProcess(schema.interviewAnswers, a as Record<string, unknown>)
			return { ...stripInternal(row, 'job_applied_id', 'sort_order'), id: row.id }
		})
	}
	if (raw.additionalAnswers?.length) {
		interview.addicional = raw.additionalAnswers.map((a) => {
			const row = postProcess(schema.additionalAnswers, a as Record<string, unknown>)
			return { ...stripInternal(row, 'job_applied_id', 'sort_order'), id: row.id }
		})
	}
...
const JA_FIELD_MAP = {
	...
	/*
	 * Aliases SEM o `.id`: metade dos call sites filtra pelo campo-ref cru
	 * (`field: 'jobApplied', value: {id}` ou value string) — no GCP isso é
	 * igualdade de referência e funciona; aqui o campo não resolvia coluna e o
	 * buildListParams DESCARTAVA o filtro em silêncio. Consequência real: o
	 * GET /interview/session devolvia a candidatura de OUTRA vaga (limit 1
	 * sem filtro) — achado no teste do plugin em 2026-08-24. O valor-ref já é
	 * normalizado pra `.id` no prepareSqlValue.
	 */
	companyOwner: schema.jobsApplied.company_id,
	jobApplied: schema.jobsApplied.post_job_id,
}
...
export function createDrizzleCandidateRepository(db: DrizzleDb): CandidateRepository {
	return {
		...
		async updateJobAppliedInTransaction(userId, id, updateFn) {
			/*
			 * Transação DE VERDADE com trava de linha.
			 *
			 * Antes era read → updateFn → write solto, com um comentário dizendo
			 * que "o PostgreSQL já tem row-level locking nativo" — tem, mas só
			 * dentro de uma transação e com a linha travada. Do jeito que estava,
			 * dois chamadores concorrentes liam o mesmo estado e o segundo
			 * sobrescrevia a decisão do primeiro, que é exatamente o que esta
			 * função existe para impedir (ex.: reivindicar o finish da entrevista).
			 */
			await db.transaction(async (tx) => {
				const locked = await tx
					.select({ id: schema.jobsApplied.id })
					.from(schema.jobsApplied)
					.where(eq(schema.jobsApplied.id, id))
					.limit(1)
					.for('update')
				if (!locked[0]) throw new Error(`JobApplied ${id} not found for user ${userId}`)

				const current = await tx.query.jobsApplied.findFirst({ where: eq(schema.jobsApplied.id, id), with: jaWith() })
				...
				if (!updates || Object.keys(updates as Record<string, unknown>).length === 0) return
				const patched = applyPatch(assembled as unknown as Record<string, unknown>, updates as Record<string, unknown>)
				await writeJobApplied(tx as unknown as DrizzleDb, id, patched, current.user_id, false)
			})
		},

...
import { createDrizzleAuditLogRepository } from "./audit-log-repository";
...
import { createDrizzleEnterpriseContractRepository } from "./enterprise-contract-repository";
import { createDrizzleEnterprisePaymentRepository } from "./enterprise-payment-repository";
...
import { createDrizzleMotorLicenseRepository } from "./motor-license-repository";
import { createDrizzleGupyIntegrationRepository } from "./gupy-integration-repository";
...
import { createDrizzleTalentCreditsRepository } from "./talent-credits-repository";
...
export function createDrizzleRepositories(db: DrizzleDb) {
	return {
		...
		gupyIntegrationRepository: createDrizzleGupyIntegrationRepository(db),
		...
		motorLicenseRepository: createDrizzleMotorLicenseRepository(db),
		aiUsageRepository: createDrizzleAiUsageRepository(db),
		adminUserRepository: createDrizzleAdminUserRepository(db),
		auditLogRepository: createDrizzleAuditLogRepository(db),
		enterpriseContractRepository: createDrizzleEnterpriseContractRepository(db),
		enterprisePaymentRepository: createDrizzleEnterprisePaymentRepository(db),
		talentCreditsRepository: createDrizzleTalentCreditsRepository(db),
	};
}

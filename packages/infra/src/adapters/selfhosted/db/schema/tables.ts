...
export const jobsApplied = pgTable(
	'jobs_applied',
	{
		...
		// da verificação (ADR-007, decisão 6).
...
// ─── MotorLicenses (lado servidor de licenças — inerte na open) ─────────────

export const motorLicenses = pgTable('motor_licenses', {
	// SHA-256 hex da chave
	id: text('id').primaryKey(),
	plan: text('plan'),
	status: text('status'),
	issuedTo: text('issuedTo'),
	notes: text('notes'),
	createdAt: timestamp('createdAt', { withTimezone: true }),
	lastSeenAt: timestamp('lastSeenAt', { withTimezone: true }),
	instance: jsonb('instance'),
})

...
import { BarChart3, Briefcase, ChevronsLeft, ChevronsRight, Plug, Search, ServerCog, Settings, Users, UsersRound, type LucideIcon, LayoutDashboard, ClipboardList, Network, Mail, ChevronDown } from 'lucide-react'
			...
			<nav className='flex-1 overflow-y-auto py-3'>
				{NAV_GROUPS.map((group) => {
					...
					const items = group.items
						.filter((item) => podeAlcancar(item.to, item.capability, item.feature, can, features))
...
function NavBranch({
	...
	const children = (item.children ?? [])
		.filter((child) => podeAlcancar(child.to, child.capability, child.feature, can, features))

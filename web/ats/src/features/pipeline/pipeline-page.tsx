...
export function PipelinePage({ jobId: fixedJobId }: { jobId?: string } = {}) {
	...
	const invitableSelection = selected.filter(
		(id) => allCards.find((c) => c.id === id)?.stage === entryStage,
	)
		...
		<div className='flex h-full flex-col'>
			...
			<div className='flex flex-wrap items-start justify-between gap-3 px-6 pb-3 pt-6'>
				...
				<div className='flex flex-wrap items-center gap-2'>
					...
					{features.motor && (
						<Button
							disabled={!jobId || (byStage.get(entryStage)?.length ?? 0) === 0}
							onClick={() => setInviting({ ids: (byStage.get(entryStage) ?? []).map((c) => c.id) })}
						>
			...
			<div className='flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-2.5'>
				...
				{(!ehFila || cards.length <= LIMIAR_DE_FILA || filaAberta) &&
					...
					<CandidateCard
						...
						showInvite={isEntry && features.motor}

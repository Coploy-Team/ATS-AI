...
import { resetMembership } from '@/lib/require-membership'
...
export function LoginPage() {
	...
	const { destino, motivo } = useSearch({ strict: false }) as { destino?: string; motivo?: string }
		...
		<div className='grid min-h-screen bg-bg lg:grid-cols-2'>
			<div className='flex items-center justify-center px-4 py-10'>
				<div className='w-full max-w-sm'>
					...
					{/* sessão derrubada por falta de vínculo com empresa (conta de
				    candidato, por exemplo) — dizer o porquê, não só pedir login */}
				{motivo === 'sem-acesso' && (
					<p className='mb-3 rounded-lg border border-amber-300/60 bg-amber-50/60 px-3 py-2.5 text-[12.5px] leading-relaxed text-amber-800 dark:bg-transparent dark:text-amber-400'>
						{t('login.noAccess')}
					</p>
				)}

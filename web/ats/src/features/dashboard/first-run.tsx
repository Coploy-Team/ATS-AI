...
export function FirstRun() {
	...
	// sem billing na edição (distribuição open) não existe saldo a buscar
	const { data: usageData } = empresa.useGetCompaniesBillingUsage({
		query: { enabled: features.billing },
	})

	const credits = (usageData?.data as { credits?: { creditsTotal?: number } } | undefined)?.credits
	const saldo = credits?.creditsTotal ?? 0

	/*
	 * A promessa muda com a edição (ADR-007). No SaaS a régua é o crédito e o

...
export function JobsPage() {
	...
	useEffect(() => {
		setPage(1)
	}, [status, creatorId, priority, find, sortBy, sortDir, advanced, types, showArchived])
				...
				<div className={cn('transition-opacity duration-150', isFetching && 'opacity-60')}>
					{view === 'table' ? (
						<JobsTable
							rows={rows}
							isLoading={isLoading}
							hasFilter={Boolean(find.trim())}
							sortHeader={SortHeader}
						/>
					) : (
						<JobsGrid rows={rows} isLoading={isLoading} hasFilter={Boolean(find.trim())} />
					)}
